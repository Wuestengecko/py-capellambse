...
class FilteringCriterionSet(capellacore.NamedElement, abstract=True):
    filtering_criteria = m.Association["FilteringCriterion"](
...
class AssociatedFilteringCriterionSet(
    FilteringCriterionSet, emde.ElementExtension
):
    pass


class CreationDefaultFilteringCriterionSet(
    FilteringCriterionSet, emde.ElementExtension
):
    pass
...
if not t.TYPE_CHECKING:
    ...
    def __getattr__(name):
        renames = {
            "AbstractResultSet": FilteringResultSet,
            "FilteringResult": AbstractFilteringResult,
        }
        if renamed := renames.get(name):
            warnings.warn(
                f"{name} has been renamed to {renamed.__name__}",
                DeprecationWarning,
                stacklevel=2,
            )
            return renamed
