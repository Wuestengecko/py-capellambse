...
impl Containment {
    ...
    fn __new__<'py>(
        ...
        let clsname = {
            let (ns, clsname) = resolve_class_name(clsname)?;
            (ns.unbind(), clsname)
        };
        ...
        let type_hint_map = if let Some(type_hint_map) = type_hint_map {
            Some(
                type_hint_map
                    .into_iter()
                    .map(|(k, cn)| {
                        let cn = resolve_class_name(cn)?;
                        Ok((k, (cn.0.unbind(), cn.1)))
                    })
                    .collect::<PyResult<_>>()?,
            )
        } else {
...
fn resolve_class_name<'py>(
    clsname: UnresolvedClassName<'py>,
) -> PyResult<(Bound<'py, Namespace>, String)> {
    let py = clsname.0.py();
...
fn find_overridden_relation<'py, R: PyTypeInfo>(
    ...
    Ok(loop {
        ...
        let Ok(rel) = rel.downcast_into() else {
            break None;
        };
        break Some(rel);
    })
